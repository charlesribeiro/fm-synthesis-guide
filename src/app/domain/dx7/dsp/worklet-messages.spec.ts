...
describe('parseWorkletMessage — hostile-payload matrix for setOperatorParameters (T-08-01)', () => {
  ...
  /** The four malformed-entry-value shapes (Phase 9, T-09-01) applied at
   * every one of `ENVELOPE_SEGMENT_COUNT` indices, for both the `rates` and
   * `levels` tuples — generated by a loop over the index range rather than
   * hand-written per-index literals, so a future index cannot be silently
   * left unchecked. */
