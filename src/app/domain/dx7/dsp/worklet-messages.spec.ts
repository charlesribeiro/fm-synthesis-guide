...
describe('parseWorkletMessage — hostile-payload matrix for setAlgorithm (T-08-01)', () => {
  const rejectedRoutingConfigPayloads: readonly { readonly name: string; readonly payload: unknown }[] = [
    ...
    {
      name: 'connections containing a duplicate feedback loop',
      payload: {
        kind: 'setAlgorithm',
        connections: [
    ...
    {
      name: 'connections containing more than one feedback edge',
      payload: {
        kind: 'setAlgorithm',
        connections: [
          { from: 6, to: 6, isFeedback: true },
          { from: 5, to: 5, isFeedback: true },
        ],
        carriers: validCarriers,
      },
    },
...
describe('parseWorkletMessage — hostile-payload matrix for setOperatorParameters (T-08-01)', () => {
  ...
  /** The malformed-entry-value shapes (Phase 9, T-09-01) applied at every
   * one of `ENVELOPE_SEGMENT_COUNT` indices, for both the `rates` and
   * `levels` tuples (2 × 4 × 7 variants plus below/above per index) —
   * generated by a loop over the index range rather than hand-written
   * per-index literals, so a future index cannot be silently left
   * unchecked. Wrong type, non-integer, NaN, Infinity and -Infinity live
   * in this table; below-minimum and above-maximum are appended per index
