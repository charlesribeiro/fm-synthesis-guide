...
provides:
  ...
  - "A demonstrated, working corruption probe: mutating Algorithm 1's shared `algorithm.edges` (reversing 4→3 to 3→4) violates the descending-order precondition and the cross-check detects the resulting GraphRouter/reference divergence; it is not a proof of a production `buildRoutingConfig` translation bug"
...
coverage:
  ...
  - id: D4
    ...
    human_judgment: false
  - id: D5
    description: "The cross-check detects divergence when a shared Algorithm 1 input is corrupted (reversing 4->3 to 3->4), violating GraphRouter's fixed descending-order precondition — 1/66 tests in algorithm-routing.spec.ts failed while the other 65 passed; the corruption was then restored and the full suite reran green (972/972). This is an invalid-topology probe, not a production buildRoutingConfig translation-bug proof (both sides still received the same mutated algorithm.edges)."
    requirement: ENGINE-02
    verification: []
    human_judgment: true
    rationale: "This is a one-time manual probe performed and observed by the executing agent during this session (not a persisted automated regression test — persisting it would require the test suite to intentionally fail on every run, which is not desired). The evidence is the transcript of the probe run and the restored-green rerun, both recorded in this summary; a human or auditor reviewing this summary is the appropriate verifier of that evidence, not an automated check. The probe proves the cross-check can fail when the shared input violates the descending-order precondition; it does not isolate a router-only RoutingConfig translation defect."
...
**An independently-derived recursive phase-modulation evaluator (`evaluateAlgorithmReference`) cross-checks all 32 canonical algorithms against `GraphRouter`'s rendered output sample-for-sample at 6 decimal places, plus proves every row stays finite and inside the hard output bound at maximum feedback and maximum output level — with a demonstrated invalid-topology corruption probe confirming the cross-check detects GraphRouter/reference divergence when the shared descending-order precondition is violated.**
...
- **The direction-reversal probe** (`3->4` instead of `4->3`) violates `validateAlgorithm`'s higher-modulates-lower structural check (expected, since this corruption is an illegal topology) and also made exactly one test in `algorithm-routing.spec.ts` fail: `Algorithm 1 ('Four-deep stack...') > matches the independent reference evaluator sample-for-sample within the declared tolerance` (1 of 66 tests in the file). The other 65 tests in the file — including Algorithm 1's own bounded-feedback-sweep case and both cross-check and sweep cases for all other 31 rows — passed. Full-suite result: 21 tests failed across 9 files (expected — the corruption is a genuinely invalid, unvalidatable topology, so many unrelated dataset-integrity tests correctly caught it too), with exactly 1 of those 21 being the targeted cross-check case. Because both implementations still received the same mutated `algorithm.edges`, this probe demonstrates the cross-check detecting divergence under a violated descending-order precondition, not a production `buildRoutingConfig` translation bug.
