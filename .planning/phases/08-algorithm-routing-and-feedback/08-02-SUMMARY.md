...
key-decisions:
  ...
  - "The corruption probe intentionally did NOT corrupt an ALGORITHMS row's edges in a way that only breaks dataset-integrity invariants (e.g. simply changing an edge target while preserving the higher-modulates-lower direction) — that class of corruption changes both GraphRouter's and evaluateAlgorithmReference's inputs identically (both read algorithm.edges directly) and produces a consistent-but-wrong result on both sides, which would NOT fail the cross-check and would only be caught by algorithms.spec.ts's own dataset invariant tests (confirmed empirically first). The probe instead reversed one edge's direction (5->4 kept, 4->3 became 3->4), which breaks GraphRouter's higher-modulates-lower/fixed-descending-order assumption while evaluateAlgorithmReference's per-sample recursion has no such ordering constraint — this is what actually exercises a GraphRouter *translation* bug rather than a dataset bug, and is the correct probe for D-10's stated purpose."
  - "getFeedbackOperator/derive-role.ts was deliberately NOT imported into algorithm-routing.spec.ts's fixture-construction helper; the feedback operator id is instead read from buildRoutingConfig's own already-computed isFeedback flag on its RoutingConfig.connections, mirroring GraphRouter.setRouting's own findFeedbackOperatorId convention exactly."
...
- `getFeedbackOperator`/`derive-role.ts` was deliberately not imported into the spec's fixture-construction helper; the feedback operator id is read from `buildRoutingConfig`'s own `isFeedback`-flagged `RoutingConfig.connections`, matching `GraphRouter.setRouting`'s own convention.
