...
- The round-trip test clicks the first rendered card (`firstCard!.click()`) so Angular's `routerLink` performs the navigation, then asserts the lesson title. It does not extract `href` or call `navigateByUrl` for that hop — that is what proves the *rendered* anchor activates correctly, not just that the route table itself is wired.
