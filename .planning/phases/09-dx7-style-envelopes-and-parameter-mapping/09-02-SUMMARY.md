...
tech-stack:
  added: []
  patterns:
    - "Index-parameterised hostile-payload generation: a loop over rates/levels x 4 indices x {wrong-type, non-integer, NaN, Infinity, -Infinity, below-min, above-max} builds the 56 malformed envelope entry-value cases (all 56 ran), rather than 56 hand-written literal cases; positive boundary cases are stated separately"
...
- **Task 1 — hostile-payload matrix + bundle parity.** `worklet-messages.spec.ts` gained a new `setGate` hostile-payload describe block (10 rejection cases + throwing-getter case + 2 positive-boundary cases, all bounds imported rather than literal) and the existing `setOperatorParameters` table gained container/tuple-length/per-index envelope entry-value cases (56 index-parameterised malformed cases via a loop over `rates`/`levels` × 4 indices × 7 malformed-value shapes — wrong type, non-integer, `NaN`, `Infinity`, `-Infinity`, below minimum, above maximum — plus positive boundary cases stated separately, plus 6 container/tuple-length cases and a throwing-getter case). `worklet-processor-bundle.spec.ts` gained a gated-note parity case proving the built worklet bundle renders the same enveloped samples as the in-repo `GraphRouter` kernel, with an explicit non-silence assertion before the parity assertion.
...
**Algorithm selection for the taxonomy sweep:** `TAXONOMY_SWEEP_ALGORITHMS` maps each `TEACHING_TAGS` entry through `ALGORITHMS.find(...)` and validates the lookup: if no algorithm carries that tag, it throws immediately with an error that includes the missing tag. Valid tags keep first-match selection in dataset order (`'additive-stacks'`, `'tree-branch'`, `'rooting'`, `'parallel'` — Algorithms 1, 7, 19, and 26 under the current dataset). No id is hardcoded; a future reordering of `ALGORITHMS` would still select one algorithm per group, and a missing tag fails at module load instead of asserting through a non-null operator.
